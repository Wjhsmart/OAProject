...
public class EmpDAOImpl extends AbstractBaseDAO<Emp> implements EmpDAO {
	...
	public Pager4EasyUI<Emp> queryByPager2(Pager4EasyUI<Emp> pager, String depId) {
		...
		Session session = sessionFactory.getCurrentSession();
		SQLQuery sqlQuery = session.createSQLQuery("select empid, name from t_emp where empid in(select empid from t_salaryinfo) and depid='" + depId + "' limit " + pager.getBeginIndex() + ", " + pager.getPageSize());
